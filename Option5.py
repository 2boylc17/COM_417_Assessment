...
def remove(basket_id):
    # Checks if there is a basket to be removed from
    if basket_id is None:
        ...
        if length > 1:
            ...
            correct = False
            item_no = 0
            # Repeatedly asks for correct item no.
            ...
            while correct is False:
                ...
                if dec == 'Y':
                    # Removes item where all IDs match
                    ...
                    sql_query = f"DELETE FROM basket_contents WHERE basket_id = {basket_id} AND \
                                            product_id = {r_product} AND seller_id = {r_seller}"
                    cursor.execute(sql_query)
                    db.commit()
                    correct = True
                elif dec == 'N':
            ...
            # Same as previous section except item_no is always 1
            print("{0:<15}{1:70}{2:30}{3:<10}{4:<10}{5:10}".format('Basket Item', 'Product Description',
                                                                   'Seller Name', 'Quantity', 'Price', 'Total'))
            overall = 0
            for num in contents:
